...
namespace RealSchoolBus.AI
{
    public class NewSchoolAI : PlayerBuildingAI
	{
        ...
        protected override void ProduceGoods(ushort buildingID, ref Building buildingData, ref Building.Frame frameData, int productionRate, int finalProductionRate, ref Citizen.BehaviourData behaviour, int aliveWorkerCount, int totalWorkerCount, int workPlaceCount, int aliveVisitorCount, int totalVisitorCount, int visitPlaceCount)
        {
            ...
            buildingData.m_customBuffer1 = (ushort)aliveCount;
            if (m_info.m_class.m_level == ItemClass.Level.Level3 && (servicePolicies & DistrictPolicies.Services.SchoolsOut) != DistrictPolicies.Services.None)
            {
            ...
            if (num3 >= 1)
            {
                TransferManager.TransferOffer offer = new TransferManager.TransferOffer
                {
                    Priority = Mathf.Max(1, num3 * 8 / num2),
                    Building = buildingID,
        ...
        public override string GetLocalizedStats(ushort buildingID, ref Building data)
        {
            ...
            return text + LocaleFormatter.FormatGeneric(localeID, global);
        }
		...
		private void HandleSchoolVehicles(ushort buildingID, ref Building data, int productionRate)
		{
			...
			if (randomVehicleInfo is not null)
			{
